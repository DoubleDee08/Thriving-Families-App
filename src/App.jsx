    ...
    <div style={{ display: "flex", minHeight: "100vh", fontFamily: SANS, color: C.ink, background: C.bg, WebkitFontSmoothing: "antialiased" }}>
      ...
      <aside style={{ width: 252, flex: "0 1 252px", minWidth: 208, background: C.sidebar, borderRight: `1px solid ${C.sidebarBorder}`, display: "flex", flexDirection: "column", padding: "26px 0 18px" }}>
        ...
        <nav style={{ display: "flex", flexDirection: "column", gap: 2, padding: "0 12px" }}>
          ...
          {["Billing", "Referrals", "Reports"].map((label) => (
            <div key={label} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, padding: "8px 10px", fontSize: 14, color: C.placeholder }}>
              {label}
              <span style={{ fontSize: 9.5, letterSpacing: "0.1em", textTransform: "uppercase", fontWeight: 700, color: C.placeholder, border: "1px solid #DFD5C6", padding: "2px 5px", borderRadius: 3 }}>Planned</span>
            </div>
          ))}
        </nav>
...
function IntakeScreen({ households, templates, packets, onNewPacket, onMarkSigned }) {
  ...
  return (
    <div style={{ padding: "26px 40px 60px", maxWidth: 1120 }}>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 9, overflow: "hidden" }}>
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 2fr) minmax(0, 1.4fr) auto", background: C.panelCard, padding: "10px 18px", fontSize: 11.5, fontWeight: 700, color: C.mutedLight, textTransform: "uppercase", letterSpacing: "0.08em" }}>
          <div>Document</div><div>Purpose</div><div>Revised</div>
        </div>
        {templates.map((t, i) => (
          <div key={t.id} style={{ display: "grid", gridTemplateColumns: "minmax(0, 2fr) minmax(0, 1.4fr) auto", padding: "14px 18px", fontSize: 13.5, borderTop: i > 0 ? `1px solid ${C.hairline}` : "none", alignItems: "center" }}>
            ...
            <div style={{ color: C.mutedLight }}>{t.revised_on}</div>
          </div>
        ))}
      </div>
